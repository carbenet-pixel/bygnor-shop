...
export default async function CustomerDetailPage({
  ...
  const resetMfaFormId = `reset-mfa-${customer.id}`;
  ...
  return (
    <div>
      ...
      <div className="mt-6 max-w-xl rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <form id={resetMfaFormId} action={resetCustomerMfaAction}>
          <input type="hidden" name="customerId" value={customer.id} />
        </form>
        <ConfirmSubmitButton
          formId={resetMfaFormId}
          confirmMessage={`Er du sikker på at nulstille 2FA for ${customer.companyName ?? "denne kunde"}? Kunden bliver logget ud og skal opsætte 2FA forfra ved næste login.`}
